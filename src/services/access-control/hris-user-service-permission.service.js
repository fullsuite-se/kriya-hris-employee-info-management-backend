...
exports.getUserServicePermission = async (user_id) => {
  const servicePermissionsRaw = await HrisUserServicePermission.findAll({
    where: { user_id },
    include: Service,
  });
...
exports.addUserServicePermission = async (user_id, service_ids) => {
  ...
  const servicePermissions = await HrisUserServicePermission.bulkCreate(
    servicePermissionsData
  );
...
exports.deleteUserServicePermission = async (user_service_permission_ids) => {
  for (const user_service_permission_id of user_service_permission_ids) {
    const servicePermission = await HrisUserServicePermission.findByPk(
