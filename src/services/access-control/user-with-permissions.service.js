...
//     // ---------- STEP 2: Get all their HrisUserInfo ----------
//     const userIds = [...new Set(servicePermissions.map(sp => sp.user_id))];

//     if (userIds.length === 0) {
//       return { users: [], counts: { allCount: 0, length: 0 } };
//     }
...
//             { [Op.and]: [{ first_name: { [Op.like]: `%${searchParts[0]}%` } }, { last_name: { [Op.like]: `%${searchParts[1]}%` } }] },
//             { [Op.and]: [{ first_name: { [Op.like]: `%${searchParts[1]}%` } }, { last_name: { [Op.like]: `%${searchParts[0]}%` } }] }
//           );
//         } else if (searchParts.length >= 3) {
//           searchConditions.push(
//             { [Op.and]: [{ first_name: { [Op.like]: `%${searchParts[0]}%` } }, { last_name: { [Op.like]: `%${searchParts[searchParts.length - 1]}%` } }] },
//             { [Op.and]: [{ first_name: { [Op.like]: `%${searchParts[searchParts.length - 1]}%` } }, { last_name: { [Op.like]: `%${searchParts[0]}%` } }] }
//           );
//         }
...
//     // ---------- STEP 3: Apply the searching process based on query ----------
